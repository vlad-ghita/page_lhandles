...
class Extension_page_lhandles extends Extension
{
    ...
    public function install()
    {
        ...
        $this->updateColumns(FLang::getLangs());
    ...
    public function dFrontendPrePageResolve($context)
    {
        if ($this->first_pass === true //1. to prevent an endless loop if called after the 404 is generated
            && $this->validateDependencies() //2. make sure needed extensions are enabled
       ) {
    ...
    public function dAddCustomPreferenceFieldsets($context)
    {
        ...
        $this->appendUpdateButton($group);
        $this->appendConsolidate($group);
        $this->appendOperatingMode($group);
     ...
     */
    private function appendUpdateButton(XMLElement &$wrapper)
    {
        $main_lang = FLang::getMainLang();
        $all_langs = FLang::getAllLangs();
     ...
     * @param XMLElement &$wrapper
     */
    private function appendOperatingMode(XMLElement &$wrapper)
    {
    ...
    public function dCustomActions()
    {
        if (isset($_POST['action'][PLH_GROUP]['update'])) {
            $this->insertTestTitlesAndHandles();
        }
    }
    ...
    {
        $this->updateColumns($context['new_langs'], $context['context']['settings'][PLH_GROUP]['consolidate']);

        $this->insertTestTitlesAndHandles($context['new_langs']);
    ...
    private function updateColumns($langs, $consolidate = 'yes')
    {
        try {
        ...
        if ($show_columns) {
            foreach ($show_columns as $column) {
                ...
                if (($consolidate !== 'yes') && !in_array($lc, $langs)) {
                    $this->query(sprintf('ALTER TABLE `%1$s` DROP COLUMN `plh_t-%2$s`;', self::DB_TABLE, $lc));
                    $this->query(sprintf('ALTER TABLE `%1$s` DROP COLUMN `plh_h-%2$s`;', self::DB_TABLE, $lc));
                } else{
     ...
     * @return boolean
     */
    private function insertTestTitlesAndHandles($langs = array())
    {
        if (empty($langs)) {
            $langs = FLang::getLangs();
